...
const UpdateReviewModal = ({ isOpen, closeModal, reviewData, setReviewData, refetch }) => {
    const { user } = useAuth();
    const axiosSecure = useAxiosSecure();
    const navigate = useNavigate();
    const [loading, setLoading] = useState(false);
    const [currentDate, setCurrentDate] = useState(new Date());
    const [rating, setRating] = useState(reviewData?.reviewRating);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const { data } = await axiosSecure.patch(`/update-review/${reviewData?._id}`, updateReviewData);
                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        ...
                        >
                            <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                                ...
                                >
                                    Update <span className='text-[#0AB99D]'>Review</span>
                                </Dialog.Title>
